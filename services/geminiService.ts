...
import { GoogleGenAI, GenerateContentResponse, Chat as GenAIChat, Content, Type } from "@google/genai";
...
export const suggestMetadata = async (content: string): Promise<{ title: string; subject: string; topic: string; difficulty: 'Easy' | 'Medium' | 'Hard' }> => {
  ...
  return metadata || { title: 'Untitled', subject: '', topic: '', difficulty: 'Medium' };
};
...
export const generatePresentationImages = async (
  ...
  onProgress: (progress: string) => void,
    ...
    for (let i = 0; i < presentation.slides.length; i++) {
        ...
        onProgress(`Creating visual for slide ${i + 1} of ${presentation.slides.length}...`);
        ...
        try {
            const imageResponse = await withRetry(() => 
                ...
                (attempt, delay) => { onProgress(`Rate limit hit on slide ${i + 1}. Retrying in ${delay / 1000}s...`); }
            );
            
            const image = imageResponse.generatedImages?.[0]?.image;
            const imageUrl = image?.imageBytes ? `data:${image.mimeType};base64,${image.imageBytes}` : undefined;
            updatedSlides.push({ ...slide, imageUrl });
        } catch (error) {
            console.error(`Error generating image for slide: "${slide.title}"`, error);
            updatedSlides.push({ ...slide, imageUrl: undefined });
        }
        
        if (i < presentation.slides.length - 1) {
            onProgress(`Waiting 12s to avoid rate limits...`);
            await sleep(12000);
        }
    }
    ...
    for (let i = 0; i < scenesData.length; i++) {
      ...
      try {
        ...
        if (image?.imageBytes) {
            ...
            generatedScenes.push({ ...sceneData, imageUrl: undefined });
        }
      ...
      if (i < scenesData.length - 1) {
        onProgress(`Waiting 12s to avoid rate limits before the next image...`);
        await sleep(12000);
      }
    }
